...
static ssize_t recv_msg(int sock, int *fd_out, void *buf, size_t buf_len) {
  ...
  ssize_t ret;
  do { ret = recvmsg(sock, &msghdr, MSG_CMSG_CLOEXEC); } while (ret < 0 && errno == EINTR);

  if (fd_out) {
    struct cmsghdr *cmsg = CMSG_FIRSTHDR(&msghdr);
    if (cmsg) memcpy(fd_out, CMSG_DATA(cmsg), sizeof(*fd_out));
    else *fd_out = -1;
  }
...
static void send_msg(int sock, int fd, void *buf, size_t buf_len) {
  ...
  ssize_t ret;
  do { ret = sendmsg(sock, &msghdr, 0); } while (ret < 0 && errno == EINTR);
}
...
int devmgr_open(int sockfd, const char *path) {
  ...
  int fd, err, ret;
  int retry = 0;
  do { ret = recv_msg(sockfd, &fd, &err, sizeof(err)); } while (ret == 0 && retry++ < 3);

...
static void render_frame(struct wsk_state *state) {
  ...
  if (state->output) cairo_font_options_set_subpixel_order(fo, to_cairo_subpixel_order(state->output->subpixel));
  ...
  if (height / scale != state->height || width / scale != state->width || state->width == 0) {
    // Reconfigure surface
    if (width == 0 || height == 0) wl_surface_attach(state->surface, NULL, 0, 0);
    else zwlr_layer_surface_v1_set_size(state->layer_surface, width / scale, height / scale);
    ...
    state->current_buffer = get_next_buffer(state->shm, state->buffers, state->width * scale, state->height * scale);
...
static void set_dirty(struct wsk_state *state) {
  if (state->frame_scheduled) state->dirty = true;
  else if (state->surface) render_frame(state);
}
...
static void layer_surface_configure(void *data, struct zwlr_layer_surface_v1 *zwlr_layer_surface_v1, uint32_t serial,
  ...
  state->width            = width;
  state->height           = height;
  // FIXME this aparently gets called in a race condition or before waiting for previous commit to finish
...
static void surface_enter(void *data, struct wl_surface *wl_surface, struct wl_output *output) {
  ...
  while (wsk_output->output != output) wsk_output = wsk_output->next;
...
static void registry_global(void *data, struct wl_registry *wl_registry, uint32_t name, const char *interface,
  ...
  if (strcmp(interface, wl_compositor_interface.name) == 0) {
    ...
    state->output_mgr = wl_registry_bind(wl_registry, name, &zxdg_output_manager_v1_interface, 1);
...
static void handle_libinput_event(struct wsk_state *state, struct libinput_event *event) {
  ...
  if (event_type != LIBINPUT_EVENT_KEYBOARD_KEY) return;
  ...
  xkb_state_update_key(state->xkb_state, keycode, key_state == LIBINPUT_KEY_STATE_RELEASED ? XKB_KEY_UP : XKB_KEY_DOWN);
...
void exit_devmgr(void) { devmgr_finish(state.devmgr, state.devmgr_pid); }

void exit_libinput(void) { libinput_unref(state.libinput); }

void exit_wayland(void) { wl_display_disconnect(state.display); }
...
int main(int argc, char *argv[]) {
  ...
  while (ret < argc && allow_root == false) {
    if (strncmp(argv[ret], ALLOW_ROOT, 14) == 0) allow_root = true;
    ret++;
  }

  if (devmgr_start(allow_root, &state.devmgr, &state.devmgr_pid, INPUTDEVPATH) > 0) return 1;
  atexit(exit_devmgr);
  ...
  while ((c = getopt(argc, argv, "hb:f:s:F:t:a:m:o:-:")) != -1) {
    switch (c) {
      ...
      case 'a':
        if (strcmp(optarg, "top") == 0) anchor |= ZWLR_LAYER_SURFACE_V1_ANCHOR_TOP;
        else if (strcmp(optarg, "left") == 0) anchor |= ZWLR_LAYER_SURFACE_V1_ANCHOR_LEFT;
        else if (strcmp(optarg, "right") == 0) anchor |= ZWLR_LAYER_SURFACE_V1_ANCHOR_RIGHT;
        else if (strcmp(optarg, "bottom") == 0) anchor |= ZWLR_LAYER_SURFACE_V1_ANCHOR_BOTTOM;
        break;
      case 'm': margin = atoi(optarg); break;
      case 'o': fprintf(stderr, "-o is unimplemented\n"); return 0;
      case '-':
  ...
  struct {
    ...
    void *ptr;
      ...
      "wl_compositor", &state.compositor, "wl_shm", &state.shm, "wl_seat", &state.seat, "wlr_layer_shell", &state.layer_shell,
  };
  ...
  state.layer_surface = zwlr_layer_shell_v1_get_layer_surface(state.layer_shell, state.surface, NULL,
  ...
  while (state.run) {
    ...
    if (now.tv_sec >= state.last_key.tv_sec + state.timeout && now.tv_nsec >= state.last_key.tv_nsec) {
      struct wsk_keypress *key = state.keys;
      while (key) {

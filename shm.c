...
void destroy_buffer(struct pool_buffer *buffer) {
  ...
  if (buffer->cairo) cairo_destroy(buffer->cairo);
  if (buffer->surface) cairo_surface_destroy(buffer->surface);
  if (buffer->pango) g_object_unref(buffer->pango);
...
struct pool_buffer *get_next_buffer(struct wl_shm *shm, struct pool_buffer pool[static 2], uint32_t width, uint32_t height) {
  ...
  if (buffer->width != width || buffer->height != height) destroy_buffer(buffer);

  if (!buffer->buffer) {
    if (!create_buffer(shm, buffer, width, height, WL_SHM_FORMAT_ARGB8888)) return NULL;
  }
  buffer->busy = true;
  return buffer;
}
